...
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    ...
    $sql = "UPDATE booksb set nameBo=?, authorBo=?, descriptionBo=? where id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($naslov, $autor, $opis, $idBook));
   ...
   /* $sql = "SELECT * FROM booksb order by id desc limit 1";
    ...
    $sql = "UPDATE ratingsb set ratingR=? where bookIdR= ? and  userIdR = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($ocjena, $idBook, $user));
    
    Database::disconnect();
//header("Location: index.php");
    echo "Uspješno ste ažurirali knjigu!";
    //}
    //else{
